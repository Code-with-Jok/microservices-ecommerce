...
Chúng ta tách biệt logic database thành một package riêng biệt tại `packages/product-db` (được định danh là `@repo/product-db`) và được sử dụng bởi `product-service`. `product-service` sẽ tiêu thụ database thông qua package này thay vì cài trực tiếp `@prisma/client`.
...
datasource db {
  provider = "postgresql"
  // URL được quản lý tập trung trong prisma.config.ts
}
```
...
```json
{
  "tasks": {
    "db:generate": {
      "inputs": ["prisma/schema.prisma", "prisma.config.ts", "package.json", "pnpm-lock.yaml"],
      "outputs": ["generated/**"],
      "cache": true
    },
    "db:migrate": {
